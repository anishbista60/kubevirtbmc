...
func desiredReplicasFor(virtualMachineBMC *bmcv1.VirtualMachineBMC) int32 {
	if virtualMachineBMC.Spec.Instance != nil {
		return *virtualMachineBMC.Spec.Instance
	}
	return defaultDesiredReplicas
}
...
func (r *VirtualMachineBMCReconciler) createVirtBMCDeployment(virtualMachineBMC *bmcv1.VirtualMachineBMC) *appsv1.Deployment {
	...
	deployment := &appsv1.Deployment{
		...
		Spec: appsv1.DeploymentSpec{
			Replicas: int32Ptr(desiredReplicasFor(virtualMachineBMC)),
...
func (r *VirtualMachineBMCReconciler) scaleVirtBMCDeploymentReplicas(ctx context.Context, virtualMachineBMC *bmcv1.VirtualMachineBMC) error {
	log := log.FromContext(ctx)
	deployment, err := r.getVirtBMCDeployment(ctx, virtualMachineBMC)
	if err != nil {
		return err
	...
	desiredReplicas := desiredReplicasFor(virtualMachineBMC)
	...
	if err := r.Update(ctx, deployment); err != nil {
		log.Error(err, "unable to scale virtBMC Deployment", "deployment", deployment, "replicas", desiredReplicas)
		return err
	}

	log.Info("scaled virtBMC Deployment", "deployment", deployment, "replicas", desiredReplicas)
	return nil
}
